...
export function EditInvoicePage() {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<FormData>({ resolver: zodResolver(schema) as any })
  ...
  const loadInvoice = async (userId: string, invoiceId: string) => {
    try {
      ...
      reset({
        ...
        currency: data.currency as 'INR' | 'USD' | 'EUR' | 'GBP' | 'CAD' | 'AUD',
        notes: data.notes || '',
        terms: data.terms || '',
    ...
    <div className="max-w-4xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex space-x-4">
          <Button type="submit" loading={saving} className="flex-1">Update Invoice</Button>
          <Button type="button" variant="secondary" onClick={() => navigate(`/app/invoices/${id}`)}>Cancel</Button>
        </div>
      </form>
    </div>
  )
